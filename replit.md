...
**CI Quality Gates** (`.github/workflows/quality-gates.yml`): Automated quality enforcement on every PR and push to main. Runs build → UI coverage check → smoke navigation tests. Uploads Playwright HTML reports and UI gap reports as artifacts. Prevents merging code that fails quality standards. All checks must pass: clean build, 100% UI coverage (27/27 pages), and all routes functional.
...
**Styleguide** (`/styleguide`): In-app design system documentation page showcasing the complete token-driven Material Design system. Displays color palette, typography scale, spacing grid (8px), border radius, elevation shadows, button styles, status badges, and card components. Includes interactive "Show/Hide CSS" toggles for each section and live testing instructions for developers. Demonstrates how changing a single CSS variable in `index.css` (e.g., `--md-primary: #1a73e8;`) instantly updates all buttons, links, and accents across the entire app. Serves as the single source of truth for all visual design tokens and component patterns.
