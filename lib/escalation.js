...
async function runEscalationTick(now = new Date()) {
  ...
  try {
    // Find incidents that need escalation using optimized index-based query
    // Uses next_due_at column and index for fast lookups
    // Safety limits:
      ...
      `SELECT *
       ...
       WHERE status = 'open'
         ...
         AND (escalated_at IS NULL OR 
              EXTRACT(EPOCH FROM ($1::timestamptz - escalated_at)) / 60.0 >= $3)
       ORDER BY next_due_at ASC`,
      [now.toISOString(), MAX_ESC_LEVEL, ESC_SNOOZE_MIN]
    );
