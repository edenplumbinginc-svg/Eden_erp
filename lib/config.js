...
const optionalBoolString = (defaultVal) =>
  z.preprocess(
    (val) => val === undefined || val === '' ? String(defaultVal) : val,
...
const configSchema = z.object({
  ...
  ESCALATION_WORKER_ENABLED: optionalBoolString(false),
  ESCALATION_V1: optionalBoolString(false),
  ESC_DRY_RUN: optionalBoolString(true),
...
function loadConfig() {
  ...
  try {
    ...
    console.error('\n❌ Configuration error:', err.message || String(err));
    process.exit(1);
  }
}
