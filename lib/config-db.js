...
function assertSingleDatabaseUrl() {
  ...
  // Warn if using transaction pooler instead of session pooler
  if (details.isTransactionPooler) {
    console.warn(`⚠️  Using transaction pooler (aws-1). Session pooler (aws-0) recommended.`);
  }
  ...
  if (EXPECTED_DB_HOST && details.host !== EXPECTED_DB_HOST) {
    ...
    console.error(`   Actual:   ${details.host}`);
    console.error('');
  ...
  // Enforce expected project ref if configured
  if (EXPECTED_DB_PROJECT_REF && details.projectRef !== EXPECTED_DB_PROJECT_REF) {
    console.error('🚫 Database project ref mismatch detected!');
    console.error(`   Expected: ${EXPECTED_DB_PROJECT_REF}`);
    console.error(`   Actual:   ${details.projectRef}`);
    console.error('');
    console.error('   This prevents accidental connections to the wrong database.');
    process.exit(1);
  }
